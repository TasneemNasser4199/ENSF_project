...
if wandb.run is None:
    wandb.init(project="unet", settings=wandb.Settings(start_method="fork"))
    wandb.run.name = 'x'
...
def test(test_loader, model, root_diru, test_csv):
    device = next(model.parameters()).device
    results_dir = os.path.join(root_diru, "test_results")
    os.makedirs(results_dir, exist_ok=True)

    # Load the best model
    best_model_path = os.path.join(root_diru, "best_model.pt")
...
def train(train_loader, val_loader, model, optimizer, scheduler, max_epochs, root_diru, start_epoch=1):
    model.train()

    #best_val_loss = float('inf')
        # Define the path to the checkpoint file
    checkpoint_path = os.path.join(root_diru, "best_model.pt")
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            save_path = os.path.join(root_diru, "best_model.pt")
        ...
        save_path = os.path.join(root_diru, "last_model.pt")
        torch.save(state, save_path)
...
def load_last_model(model, optimizer, scheduler, root_diru, reset_lr=None):
    # Load the last model weights
    last_model_path = os.path.join(root_diru, "last_model.pt")
